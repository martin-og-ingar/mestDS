...
#             self.image(tmpfile.name, x=30, w=150)
...
class PDF(FPDF):
    ...
    #     # Calculate figure size and scale
    #     fig_width_mm = 11.7 * 25.4  # ~297 mm
    #     fig_height_mm = 6.5 * 25.4  # ~165 mm

    #     page_width_mm = self.w - 2 * self.l_margin
    #     page_height_mm = self.h - self.b_margin - self.t_margin  # full printable height

    #     scale_factor = 0.9  # scale down plot to 90% width
    #     image_width_mm = page_width_mm * scale_factor
    #     aspect_ratio = fig_height_mm / fig_width_mm
    #     image_height_mm = image_width_mm * aspect_ratio

    #     # Get current Y position after table to place plot below that
    #     current_y = self.get_y()

    #     # Calculate max Y to keep plot on page
    #     max_y_for_plot = page_height_mm - image_height_mm - 10  # 10mm bottom margin

    #     # If current_y is too low, move plot higher so it fits
    #     if current_y > max_y_for_plot:
    #         y_position = max_y_for_plot
    #     else:
    #         y_position = current_y + 5  # small padding below table
    ...
    # def add_subheader_table_and_plot(
    #     self, subheader: str, metrics: list[LossMetrics], figure
    # ):
    #     metrics = list(metrics)
    #     # Estimate total height needed: subheader (12), table (10 * rows + 10), plot (~90)
    #     table_height = 10 * (len(metrics) + 1)  # +1 for header row
    #     total_height_needed = 12 + table_height + 90

    #     self.ensure_space(total_height_needed)
